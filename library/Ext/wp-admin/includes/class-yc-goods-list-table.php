<?php
/**
 * List Table API: YC_Goods_List_Table class
 *
 * @package WordPress
 ...
 * @see WP_List_Table
 */
class YC_Goods_List_Table extends WP_List_Table {
...
//			echo "\n\t" . $this->single_row( $user_object, '', '', isset( $post_counts ) ? $post_counts[ $userid ] : 0 );
			echo '<tr>';
			echo '<td><a href="' . admin_url() . 'admin.php?page=goods-detail&goods='. $object->goods. '&action=edit">'. sprintf('%07d', $object->goods). '</a></td>';
			$separately = ($object->separately_fg == true) ? mb_convert_encoding(" （バラ）", "UTF-8", "SJIS"): null;
			echo '<td>'. $object->name. $separately. '</td>';
