...
//			echo '<td><a href="/wp-admin/admin.php?page=lot-regist&sales='. $object->sales. '&goods='. $object->goods. '&action=save">'. $object->qty. '</a></td>';
			...
			if ($object->status == '0') {
				echo '<td><span class="text-danger">'. mb_convert_encoding("–¢Šm’è", "UTF-8", "SJIS"). '</span></td>';
//				echo '<td><span class="text-danger">{{ $initForm["select"]["status"][$object->status] }}</span></td>';
			} else {
				echo '<td><span class="text-success">'. mb_convert_encoding("Šm’è", "UTF-8", "SJIS"). '</span></td>';
	...
	public function get_column_info() {
		return array(
			array(
				...
				'sales' => mb_convert_encoding('’•¶”Ô†', 'UTF-8', 'SJIS'), 
				'name' => mb_convert_encoding('’•¶ŽÒ–¼', 'UTF-8', 'SJIS'), 
				'goods' => mb_convert_encoding('¤•i', 'UTF-8', 'SJIS'), 
				'qty' => mb_convert_encoding('ŒÂ”', 'UTF-8', 'SJIS'), 
				'' => mb_convert_encoding('oŒÉ‘qŒÉ', 'UTF-8', 'SJIS'), 
				'arrival_dt' => mb_convert_encoding('ˆøŽæ(“üŒÉ)—\’è“ú', 'UTF-8', 'SJIS'), 
