	...
	<div class="row mb-3">
		...
		<input type="text" class="col-sm-2 col-form-control" id="goods_name" name="goods_name" aria-describedby="goodsNameHelp" value="{{$rows->name}}" @if ($get->action != '' && $get->action != 'save' && $get->action != 'edit') readonly @endif>
<!--		<div id="orderName" class="form-text">商品名を入力してください。</div>-->
	</div>

	<div class="row mb-3">
		<label for="qty" class="col-sm-2 col-form-label">内容量(t)</label>
		<input type="text" class="col-sm-2 col-form-control" id="qty" name="qty" aria-describedby="qtyHelp" value="{{$rows->qty}}" @if ($get->action != '' && $get->action != 'save' && $get->action != 'edit') readonly @endif>
	</div>
